...
def append_balance_change(part: dict, today: dict):
    ...
    if 'fmBalChan24' in today:
        ...
        change = nan
    part['mail'].append(fm_bal)
    part['csv'].append("{};{}".format(round(today['fmBal']), change))#!/usr/bin/python3
import calendar
import configparser
import datetime
import inspect
...
class Order:
    ...
    def __str__(self):
        return "{} order id: {}, price: {}, amount: {}, created: {}".format(self.side, self.id,
                                                                            round(self.price) if hasattr(self, 'price') and self.price else self.undefined,
